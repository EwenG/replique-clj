...
/**
 * Clojure 1.12.5's ORIGINAL LispReader, kept verbatim (only the class name is changed) and
 * compiled test-scope only.
 *
 * <p>This is the differential-test oracle. The reader tests assert that the new Buffer-backed
 * {@link LispReader} reads exactly what upstream Clojure reads: same value, same type, same
 * metadata, same exceptions. Without this class the tests would oracle the new reader against
 * RT.readString - which now IS the new reader - and would pass vacuously.
 *
 * <p>Do not edit this to make a test pass. If the two readers disagree, the new reader is wrong,
 * unless upstream behaviour is being deliberately changed - in which case say so in the test.
 */
public class OriginalLispReader{
...
static private Object installPlatformFeature(Object opts) {
    if(opts == null)
        return RT.mapUniqueKeys(OriginalLispReader.OPT_FEATURES, PLATFORM_FEATURES);
    else {
        IPersistentMap mopts = (IPersistentMap) opts;
        Object features = mopts.valAt(OPT_FEATURES);
        if (features == null)
            return mopts.assoc(OriginalLispReader.OPT_FEATURES, PLATFORM_FEATURES);
        else
            return mopts.assoc(OriginalLispReader.OPT_FEATURES, RT.conj((IPersistentSet) features, PLATFORM_KEY));
    }
}
...
public static void main(String[] args) throws Exception{
	...
	Object input = OriginalLispReader.read(rdr, false, new Object(), false );
...
public static void main(String[] args){
	...
	try
		...
		for(; ;)
			{
			ret = OriginalLispReader.read(r, true, null, false);
			RT.print(ret, w);
			w.write('\n');
			if(ret != null)
