...
// Class.getMethods() copies the whole public-method array *and every Method object in it* on each
// call, so a class with 80 public methods allocates 80 Methods per call. The compiler calls
// getMethods once per interop form it analyses (StaticMethodExpr/InstanceMethodExpr construction),
// which made Class.copyMethods the single hottest leaf under analyze — ~4.5% of compile time.
//
// ClassValue keys on the Class itself, so a class redefined at the REPL is a distinct key rather
// than a stale hit, and entries are collected with the class instead of pinning its loader.
//
// Caching costs the isolation Class.getMethods() was paying for: the Method objects handed out are
// now shared instances rather than per-call copies, so a caller that mutated one (setAccessible)
// would affect every later caller. getMethods is therefore package-private (it was public) — every
// caller is now clojure.lang.Compiler or Reflector itself, none of which mutate them, and the
// shared instances cannot escape the package. The cached array itself never escapes either:
// getMethods only reads it and returns a fresh List.
//
// Nothing generated by the compiler calls getMethods — emitted reflective call sites go to
// Reflector.invokeInstanceMethod / invokeNoArgInstanceMember — so narrowing it does not affect
// already-AOT-compiled code. It is an API break only for third-party code calling
// clojure.lang.Reflector/getMethods directly, which is a far smaller surface than the Java 21
// floor this fork already takes.
...
static List<Method> getMethods(Class c, int arity, String name, boolean getStatics){
	Method[] allmethods = PUBLIC_METHODS.get(c);
